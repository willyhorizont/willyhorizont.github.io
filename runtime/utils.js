((root, factory) => {
    ...
    const prettyFormatDate = ({ includeSecond = true, includeMiliSecond = false, fullYear, zeroPaddedMonth, monthThreeFirstLetter, zeroPaddedDay, dayThreeFirstLetter, zeroPaddedHourTwelveHourClock, twelveHourClockLatinAbbreviation, zeroPaddedHourTwentyFourHourClock, zeroPaddedMinute, zeroPaddedSecond, zeroPaddedMiliSecondThreeDigit }) => (`${zeroPaddedMonth} / 12 months | ${zeroPaddedDay} / ${new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate()} days | ${dayThreeFirstLetter}, ${zeroPaddedDay} ${monthThreeFirstLetter} ${fullYear} | ${zeroPaddedHourTwentyFourHourClock}:${zeroPaddedMinute}${includeSecond ? `:${zeroPaddedSecond}` : ``}${includeMiliSecond ? `.${zeroPaddedMiliSecondThreeDigit}` : ``} | ${zeroPaddedHourTwelveHourClock}:${zeroPaddedMinute}${includeSecond ? `:${zeroPaddedSecond}` : ``}${includeMiliSecond ? `.${zeroPaddedMiliSecondThreeDigit}` : ``} ${twelveHourClockLatinAbbreviation}`); /* prettyFormatDateV2 */
    const getMinutesDifference = (timeStrAa, timeStrBb) => ((msDiff) => (Math.floor(msDiff / (1000 * 60))))(Math.abs(parseInt(timeStrBb, 10) - parseInt(timeStrAa, 10)));

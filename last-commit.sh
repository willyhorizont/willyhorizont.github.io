...
RD=$(realpath "$SD")
V="2.3.10" # ! DON'T FORGET TO CHANGE VERSION BEFORE RUNNING !!!!
T=$(date "+%d %b %Y @ %I:%M %p")
cd "$RD" || exit
...
M="
update linux-mintx/lemonbar-simple-resource-monitor.sh;
update mindora-linux/yabar-simple-resource-monitor.sh;
"
M=$(sed -e '/./,$!d' <<< "$M")
M="$H
$M"
